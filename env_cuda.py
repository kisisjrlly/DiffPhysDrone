...
class Env:
    ...
    def _key_cue_artifacts(self, raw, scene_mask, power, exposure, gain, quality, valid_prob, hard_valid, depth_obs):
        """Apply lightweight slit/back-wall artifacts.

        Glare is an opening effect and may corrupt the whole slit cue.  For
        dark/specular, the material lives on the front-wall side patches; only
        a narrow aperture-edge/matching support term is allowed to disturb the
        slit/back-wall cue.
        """
        B = raw.shape[0]
        ...
        dark_recovery = (
            torch.sigmoid((e01 - 0.62) / 0.070)
            * torch.sigmoid((g01 - 0.52) / 0.075)
        ...
        raw_near_wide = -F.max_pool2d(-raw4, 25, stride=1, padding=12)[:, 0]
        far_like_wide = torch.sigmoid((raw - (raw_near_wide + 0.35)) / 0.060)
        ...
        side4 = side_mask[:, None]
        side_support = F.max_pool2d(side4, 9, stride=1, padding=4)[:, 0]
        side_support_wide = F.max_pool2d(side4, 25, stride=1, padding=12)[:, 0]
        aperture_edge = (cue_mask * side_support).clamp(0.0, 1.0)
        aperture_body = (cue_mask * (0.35 + 0.65 * side_support_wide)).clamp(0.0, 1.0)
        cue_area = cue_mask.mean(dim=(-2, -1), keepdim=True)
        tiny_cue = torch.sigmoid((0.030 - cue_area) / 0.008)
        material_shape = (0.70 + 0.20 * local_edge + 0.10 * texture).clamp(0.0, 1.0)
        aperture_shape = (
            aperture_edge * (0.62 + 0.28 * local_edge + 0.10 * texture)
            + 0.45 * aperture_body * (0.35 + 0.65 * texture)
        ).clamp(0.0, 1.0)
        ...
        dark_aperture_artifact = (aperture_shape * dark_material_bad * strength).clamp(0.0, 1.0)
        spec_aperture_artifact = (aperture_shape * spec_material_bad * strength).clamp(0.0, 1.0)
        aperture_artifact = torch.where(sid == 1, spec_aperture_artifact, torch.where(sid == 2, dark_aperture_artifact, torch.zeros_like(raw)))
        material_artifact = torch.where(sid == 1, spec_material_artifact, torch.where(sid == 2, dark_material_artifact, torch.zeros_like(raw)))
        artifact = torch.where(sid == 0, glare_artifact, torch.maximum(material_artifact, aperture_artifact))

        cue_quality_glare = quality - glare_artifact * (0.58 + 0.34 * glare_bad)
        cue_quality_dark = (
            quality
            - dark_material_artifact * (1.15 + 0.85 * dark_material_bad)
            - dark_aperture_artifact * (0.80 + 0.45 * dark_material_bad)
        )
        spec_recovery = side_mask * spec_safe * (1.0 - 0.55 * spec_bloom) * 0.22
        cue_quality_spec = (
            quality
            + spec_recovery
            - spec_material_artifact * (1.80 + 1.10 * spec_material_bad)
            - spec_aperture_artifact * (0.85 + 0.45 * spec_material_bad)
        )
        cue_quality = torch.where(
            sid == 0,
            cue_quality_glare,
            torch.where(sid == 1, cue_quality_spec, cue_quality_dark),
        ).clamp(0.0, 1.0)
        ...
        body_dropout_hard = (body_dropout_prob > 0.24).to(raw.dtype)
        body_dropout = body_dropout_hard.detach() - body_dropout_prob.detach() + body_dropout_prob
        valid_st = valid_st * (1.0 - body_dropout)
        tiny_keep_prob = (
            torch.maximum(cue_mask, far_like_wide)
            * tiny_cue
            * torch.sigmoid((0.86 - texture) / 0.040)
        ).clamp(0.0, 1.0)
        tiny_keep_hard = (tiny_keep_prob > 0.12).to(raw.dtype)
        tiny_keep = tiny_keep_hard.detach() - tiny_keep_prob.detach() + tiny_keep_prob
        valid_st = torch.maximum(valid_st, tiny_keep)
        ...
        false_mix = (0.62 * spec_wrong + 0.95 * material_wrong).clamp(0.0, 1.0)
        false_depth = torch.lerp(raw, wide_mixed_depth, false_mix)
        false_depth = false_depth.clamp_min(float(self.depth_min_valid))
        raw_with_false = torch.where(((sid == 1) | (sid == 2)) & ((spec_wrong + material_wrong) > 0.08), false_depth, raw)
        raw_with_false = torch.where(tiny_keep > 0.5, raw, raw_with_false)
    ...
    def _scene_mask(self, depth):
        ...
        def _side_wall_patch_mask():
            ...
            patch_width_y = _batch_scalar(
                effects.get('side_effect_width_y'),
                self.simple_slit_side_effect_width_y,
            )
            ...
            R_scene_T = self.R_scene_T.detach().to(depth.device, depth.dtype)
            R_cam_scene = torch.bmm(R_scene_T, R_cam_world)
            pos_local = torch.bmm(R_scene_T, pos[:, :, None])[:, :, 0]
            ...
            wall_gate = torch.sigmoid(
                ((wall_half_x[:, None, None] + 0.020) - (hit_x - wall_x[:, None, None]).abs())
                / 0.014
            )
            side_soft = softness.clamp_min(0.020)
            dy_from_slit_edge = (hit_y - slit_center_y[:, None, None]).abs() - slit_half_y[:, None, None]
            outside_slit = torch.sigmoid(dy_from_slit_edge / side_soft[:, None, None])
            inside_patch = torch.sigmoid(
                (patch_width_y[:, None, None] - dy_from_slit_edge)
                / side_soft[:, None, None]
            )
            patch_y = outside_slit * inside_patch
    ...
    def _scene_hit_masks(self, depth):
        ...
        fov_y = fov_x * float(H) / float(max(W, 1))
        R_scene_T = self.R_scene_T.detach().to(device, dtype)
        R_cam_scene = torch.bmm(R_scene_T, R_cam_world)
        pos_local = torch.bmm(R_scene_T, pos[:, :, None])[:, :, 0]
