...
| --- | --- | --- | --- |
| `S1` | `glare` | 开口外侧强 IR glare，测试低 exposure + 高 power 的主动恢复 | 已接入 |
| `S2` | `specular` | 同一开口区域强反射，高 power 可能 washout | 已接入 |
| `S3` | `dark` | 同一开口区域低反射，测试 exposure/gain 与 power 的权衡 | 已接入 |

## 训练课程矩阵

| 阶段 | 目标 | 相机控制 | 里程计 | 训练场景 | 退出准则 |
| --- | --- | --- | --- | --- | --- |
| `T0` | 学会在稳定深度输入下飞行 | 固定 `power/exposure/gain` | 开启 | `S1` 的低强度设置 | 成功率稳定且无明显深度崩塌 |
