...
void odom_callback(const nav_msgs::Odometry::ConstPtr &msg) {
  ...
  if ((msg->header.stamp - prevt).toSec() > 0.1 &&
      (msg->header.stamp - first_odom_time).toSec() > 2.0) {
...
void Syncallback(const nav_msgs::Odometry::ConstPtr       &msg_me,
                 const nav_msgs::Odometry::ConstPtr       &msg_other,
  ...
  rot_me = Eigen::Quaterniond(msg_me->pose.pose.orientation.w, msg_me->pose.pose.orientation.x,
                              msg_me->pose.pose.orientation.y, msg_me->pose.pose.orientation.z)
               .matrix();
  rot_other =
      Eigen::Quaterniond(msg_other->pose.pose.orientation.w, msg_other->pose.pose.orientation.x,
                         msg_other->pose.pose.orientation.y, msg_other->pose.pose.orientation.z)
          .matrix();
...
int main(int argc, char **argv) {
  ...
  message_filters::Subscriber<nav_msgs::Odometry> sub_quadstate_other(n, sub_quadpose_topic,
                                                                      1000);  // 队友
  message_filters::Subscriber<sensor_msgs::PointCloud2> sub_pointcloud(n, sub_pointcloud_topic,
                                                                       1000);  // 扫到的点云
  typedef message_filters::sync_policies::ApproximateTime<nav_msgs::Odometry, nav_msgs::Odometry,
                                                          sensor_msgs::PointCloud2>
                                              MySyncPolicy;
  message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(1000), sub_quadstate_me,
                                                   sub_quadstate_other,
