...
void MultiMapManager::insertChunkToMap(const MapChunk& chunk, const int& drone_id) {
  ...
  for (int i = 0; i < chunk.voxel_adrs_.size(); ++i) {
    ...
    // logic here is not perfect, should also check if the self map occupied
    if(chunk.voxel_occ_[i] == 1) {
      map_->md_->occupancy_buffer_[adr] = map_->mp_->clamp_max_log_;
    }else if(map_->md_->occupancy_buffer_[adr] >= map_->mp_->min_occupancy_log_)
    {
      // do nothing
    }else
    {
      map_->md_->occupancy_buffer_[adr] = map_->mp_->clamp_min_log_;
    }
