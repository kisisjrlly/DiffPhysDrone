  ...
  switch (state_) {
    ...
    case IDLE: {
      ...
      if (check_interval > 10.0) {
        ...
        ROS_WARN("drone %d go back", expl_manager_->ep_->drone_id_);
        // if (getId() == 1) {
        ...
        fd_->go_back_ = true;
        transitState(PLAN_TRAJ, "FSM"); // return after idle
        // transitState(FINISH, "FSM"); // finish, no return
        // } else {
    ...
    case PLAN_TRAJ: {
      if (fd_->static_state_) {
        ...
        double cur_traj_duration = info->position_traj_.getTimeSum();
        if(t_r > cur_traj_duration)
        { t_r = cur_traj_duration;}
...
int FastExplorationFSM::callExplorationPlanner() {
  ...
  if (frontier_callback_flag == true) {
    ROS_WARN("frontier_callback_flag == 1");
    return FAIL;
  }
...
void FastExplorationFSM::visualize(int content) {
  ...
  if (content == 1) {
    ...
    for (int i = 0; i < ed_ptr->frontiers_.size(); ++i) {
      double resolution = expl_manager_->sdf_map_->getResolution();
      visualization_->drawCubes(ed_ptr->frontiers_[i], resolution,
    ...
    for (int i = ed_ptr->frontiers_.size(); i < last_ftr_num; ++i) {
      double resolution = expl_manager_->sdf_map_->getResolution();
      visualization_->drawCubes({}, resolution, Vector4d(0, 0, 0, 1), "frontier", i, 4);
...
//        ed_ptr->points_, ed_ptr->averages_, 0.03, Vector4d(1, 0, 0, 1), "point-average", 0, 6);
    ...
    visualization_->drawBspline(info->position_traj_, 0.2,
        PlanningVisualization::getColor(
            (expl_manager_->ep_->drone_id_ - 1) / double(expl_manager_->ep_->drone_num_)),
...
void FastExplorationFSM::clearVisMarker() {
  ...
  visualization_->drawLines({}, 0.1, Eigen::Vector4d(0, 1, 0, 1), "astar", 0, 6);
  visualization_->drawSpheres({}, 0.3, Eigen::Vector4d(0, 1, 0, 1), "grid_tour", 1, 6);
...
void FastExplorationFSM::frontierCallback(const ros::TimerEvent& e) {
  ...
  if (state_ == WAIT_TRIGGER) {
    ...
    for (int i = 0; i < ed->frontiers_.size(); ++i) {
      double resolution = expl_manager_->sdf_map_->getResolution();
      visualization_->drawCubes(ed->frontiers_[i], resolution,
  ...
  frontier_callback_flag = false;

  string state_str = to_string(getId()) + "_" + fd_->state_str_[int(state_)] ;
  visualization_->drawText(fd_->odom_pos_, state_str, 0.5, Vector4d(0, 0, 1, 1), "drone_state", 0, 6);
}
...
void FastExplorationFSM::optTimerCallback(const ros::TimerEvent& e) {
  ...
  ROS_WARN("[Drone %d]Pair opt %d & %d",getId(), getId(), select_id);
  ...
  std::cout << "cur cost : " << cur_app1 << ", " << cur_app2 << ", " << cur_app1 + cur_app2
            << std::endl;
  if (cur_app1 + cur_app2 > prev_app1 + prev_app2 + 0.3) {
...
void FastExplorationFSM::optMsgCallback(const exploration_manager::PairOptConstPtr& msg) {
  ...
  std::cout << "[Drone" << getId() << "] receive opt msg from " << msg->from_drone_id << std::endl;
