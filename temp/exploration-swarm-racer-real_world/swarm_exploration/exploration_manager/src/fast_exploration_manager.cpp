...
int FastExplorationManager::planTrajToView(const Vector3d& pos, const Vector3d& vel,
  ...
  ROS_WARN_STREAM("[Drone " << ep_->drone_id_ << "]Size before shorten " << ed_->path_next_goal_.size());
  shortenPath(ed_->path_next_goal_);
  ROS_WARN_STREAM("[Drone " << ep_->drone_id_ << "]Size after shorten " << ed_->path_next_goal_.size());
  ...
  if (len < radius_close || optimistic && ed_->path_next_goal_.size() >= 3) {
  ...
  } else if (len > radius_far && ed_->path_next_goal_.size() >= 3) {
  ...
  ROS_WARN_STREAM("[Drone " << ep_->drone_id_ << "]plan done");
...
void FastExplorationManager::shortenPath(vector<Vector3d>& path) {
  ...
  if (path.size() == 1) {
    ROS_ERROR_STREAM("[Drone " << ep_->drone_id_ << "]Path only one pt! Maybe dead.");
    return;
  }
  if (path.size() == 2) {
    ROS_ERROR_STREAM("[Drone " << ep_->drone_id_ << "]Path only TWO pt! Insert!");
    Eigen::Vector3d mid_pt = (path[0] + path[1]) / 2.0;
    vector<Eigen::Vector3d> tmp_path = {path.front()};
    tmp_path.push_back(mid_pt);
    tmp_path.push_back(path.back());
    path = tmp_path;
    return;
  }
  if (path.size() <= 3) {
    ROS_ERROR_STREAM("[Drone " << ep_->drone_id_ << "]Path size too small " << path.size());
    return;
  }
