...
自然界的飞行生物展现出一种令人惊叹的协同能力：它们能在极速俯冲时收缩瞳孔、调节动态视场，并同时改变身体形态以穿过复杂的几何空间。然而，现代机器人系统通常将感知（相机参数）、控制（推力与姿态）和形态（几何通过性）视为独立的模块，导致在极端环境（如极暗、极窄）下系统失效。本文提出了一种 **“神经-物理流体 (Neuro-Physical Fluid)”** 框架，通过构建一个包含光学物理与飞行力学的统一可微流形，将端到端神经网络的隐式表征能力与**可微模型预测控制 (dMPC)** 的物理严谨性深度耦合。我们将机器人的所有自由度——从 IMX477 的底层电子变焦 (ROI 裁剪)、曝光时间到无人机的飞行姿态——视为单一能量最小化问题的耦合变量。在完全可微的仿真环境中进行端到端训练，我们证明了在无需人工启发式规则的情况下，无人机能自发涌现出“侧身穿缝”、“急停注视”和“光学呼吸”等复杂的类生物行为。这一发现表明，极具适应性的具身智能并非必须由复杂的逻辑堆砌而成，而是可以通过“黑盒视觉意图”与“白盒物理定律”的结合，在统一的物理约束场中顺梯度自然流淌而出。
...
* **现象**：基于 IMX477 硬件裁剪的视场角 (FOV) 随障碍物密度剧烈震荡。
...
其中飞行动作由 dMPC 解释执行；相机动作对 IMX477 参数做增量更新并裁剪到物理可行区间。
...
我们构建从相机参数到“感知质量”的可微算子，模拟 IMX477 电子变焦机制。训练阶段的图像由
...
## 2.7 IMX477 与 PMD flexx2 的非对称协同 (Asymmetric Sensor Roles)

### 2.7.1 主从关系

* **IMX477 主导“感知意图”**：识别远处缝隙拓扑、估计光照状态、驱动相机参数调节。
* **PMD flexx2 主导“物理安全”**：构建近场局部 SDF，提供实时几何排斥兜底。

### 2.7.2 职责分工

* IMX477：语义拓扑 + 光学反馈 + 可微感知训练主通道。
* PMD flexx2：即时测距 + $\nabla\text{SDF}$ + 最后时刻防撞约束。

### 2.7.3 参数调控对象

策略网络输出的光学动作（$t_{exp}$、ROI、gain）**只写入 IMX477 寄存器**；ToF 保持全视场稳定运行，作为独立安全层而非被动变焦器件。
...
`IMX477/IMU/ToF 前向输入 → Policy 前向意图 → IMX477 参数写寄存器 + C++ dMPC 求解 → 电机指令`
...
1. **RGB 分支（IMX477）**：轻量 CNN（MobileNetV3-small/EfficientNet-lite）提取远场语义与缝隙拓扑特征 $\mathbf{f}_{rgb}$。
...
* **IMX477 采样频率**：30 Hz（每 2 帧触发一次控制更新）。
...
3. **Figure 4 (Sim-to-Real)**: 在真实世界搭建泡沫板窄缝。展示单目无人机视角（IMX477 录像），可以看到在靠近缝隙时，画面突然**变亮**（曝光增加）、视野**瞬间收缩拉近**（ROI 电子变焦）、机身**变稳**（速度降低），然后视界**旋转90度**（物理侧身）。
