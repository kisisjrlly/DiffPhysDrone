...
| --- | --- | --- | --- |
| `S0` | `random_base` | 在混合随机几何下的一般导航与避障 | 已可用 |
| `S1` | `sun_glare` | 环境红外淹没与主动感知 power/exposure 权衡 | 当前渲染器部分支持 |
| `S2` | `specular_trap` | 高反射或类玻璃失效，预期应触发功率抑制 | 镜面部分已存在，类玻璃部分仍较弱 |
| `S3` | `black_gap` | 暗色低反照率开口，模糊-噪声权衡很关键 | 部分支持 |
| `S4` | `dark_slit_lite` | 不启用 dMPC 或 intent 的狭窄暗通道 | 尚无专门基准 |
