...
def _build_trajectory(env, xs: list[float], *, y_mode: str, target_mode: str,
    ...
    wall_x = float(fx.get("geometry_wall_x", fx.get("geometry_gate_x", 0.0)))
    goal_x = float(fx.get("geometry_goal_x", getattr(env, "simple_goal_x", 1.5)))
    gate_z = float(fx.get("geometry_gate_z", getattr(env, "simple_gate_z", 1.5)))
    x_min = min(xs)
    ...
    for idx, x in enumerate(xs):
        ...
        pos_world = _local_to_world(env, (float(x), float(y), gate_z))
        ...
        target_world = _local_to_world(env, (target_local_x, slot_y, gate_z))
